...
fn code_review_fixture() {
    ...
    let synth_pos = md
        .find("Step: synthesise")
        .expect("synthesise step missing");
    assert!(
...
fn conditional_context_preserved() {
    ...
    assert!(
        ...
        "when guard should be emitted as condition annotation"
    );
    assert!(
...
fn full_featured_fixture() {
    let fixture_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/tests/fixtures/full_featured.agent"
    );
    ...
    assert_eq!(
        ast.type_defs.len(),
        0,
        "types are imported, not locally defined"
    );
    ...
    assert_eq!(
        ...
        1,
        "expected 1 orchestration (team-review)"
    );
    ...
    assert!(
        skill.permissions.is_some(),
        "skill should have a permissions block"
    );
    assert_eq!(
        skill.includes,
        vec!["observability"],
        "skill should include observability mixin"
    );
    ...
    let body = &skill.body;
    assert_eq!(
        body.lazy_contexts.len(),
        2,
        "body should have 2 lazy contexts"
    );
    assert_eq!(body.steps.len(), 3, "body should have 3 steps");
    assert!(
        body.directives.reasoning.is_some(),
        "body should have reasoning directive"
    );
    assert!(
        body.directives.persona.is_some(),
        "body should have persona directive"
    );
    assert!(
        body.directives.sampling.is_some(),
        "body should have sampling directive"
    );
    assert_eq!(
        body.directives.reinforcements.len(),
        1,
        "body should have 1 reinforcement"
    );

    // Verify lazy context names
    let lazy_names: Vec<&str> = body
        .lazy_contexts
        .iter()
        .map(|lc| lc.name.as_str())
        .collect();
    assert!(
        lazy_names.contains(&"style-guide"),
        "should have style-guide lazy context"
    );
    assert!(
        lazy_names.contains(&"error-catalog"),
        "should have error-catalog lazy context"
    );
    ...
    assert_eq!(
        tools.required.len(),
        3,
        "should have 3 required tools (Bash, Read, mcp(github))"
    );
    assert_eq!(
        tools.optional.len(),
        1,
        "should have 1 optional tool (mcp(slack))"
    );
    ...
    assert!(
        md.contains("name: full-review"),
        "frontmatter should have skill name"
    );
    assert!(md.contains("Tools"), "should have Tools section");
    assert!(
        md.contains("Permissions"),
        "should have Permissions section"
    );
    assert!(
        md.contains("Preconditions"),
        "should have Preconditions section"
    );
    assert!(
        md.contains("References"),
        "should have References (lazy contexts) section"
    );
    assert!(
        md.contains("Reasoning mode"),
        "should have Reasoning mode directive"
    );
    assert!(
        md.contains("senior code reviewer"),
        "persona text should appear"
    );
    assert!(
        md.contains("Step: analyze"),
        "should have analyze step section"
    );
    assert!(
        md.contains("Step: deep_review"),
        "should have deep_review step section"
    );
    assert!(
        md.contains("Step: synthesise"),
        "should have synthesise step section"
    );
    assert!(
    ...
    let analyze_pos = md.find("Step: analyze").expect("analyze step missing");
    let deep_review_pos = md
        .find("Step: deep_review")
        .expect("deep_review step missing");
    let synthesise_pos = md
        .find("Step: synthesise")
        .expect("synthesise step missing");
    assert!(
    ...
    assert!(
        md.contains("style-guide"),
        "style-guide lazy context should appear"
    );
    assert!(
        md.contains("error-catalog"),
        "error-catalog lazy context should appear"
    );

    // Compile pipeline
    let pipeline_md = compiler.compile_pipeline(&ast.pipelines[0]);
    assert!(
        pipeline_md.contains("Pipeline: ci-review"),
        "should have pipeline title"
    );
    assert!(
        pipeline_md.contains("Stage: lint"),
        "should have lint stage"
    );
    assert!(
        pipeline_md.contains("Stage: security"),
        "should have security stage"
    );
    assert!(
        pipeline_md.contains("Stage: review"),
        "should have review stage"
    );
    assert!(
        pipeline_md.contains("30m"),
        "should include pipeline timeout"
    );

    // Compile orchestration
    let orch_md = compiler.compile_orchestration(&ast.orchestrations[0]);
    assert!(
        orch_md.contains("Orchestration: team-review"),
        "should have orchestration title"
    );
    ...
    assert!(
        orch_md.contains("1h"),
        "should include orchestration timeout"
    );
}
...
fn checker_reports_all_errors_not_just_first() {
    ...
    assert!(
        errs.len() >= 4,
        "Expected at least 4 errors (2 unknown types + 2 unknown steps), got {}",
        errs.len()
    );
}
...
fn all_example_skills_pass_check() {
    for path in &[
        ...
        let base_dir = std::path::Path::new(path)
            ...
            .to_path_buf();
        let mut checker = Checker::with_base_dir(base_dir);
        checker.check(&ast).unwrap_or_else(|errs| {
            panic!(
                "{}: checker failed with {} errors: {:?}",
                path,
                errs.len(),
                errs
            )
...
fn multi_file_import_resolves_types() {
    ...
    assert_eq!(
        ast.type_defs.len(),
        0,
        "types should NOT be locally defined"
    );
    assert!(
...
fn multi_file_import_missing_symbol_errors() {
    ...
    assert!(
        result.is_err(),
        "should fail — NonExistent is not in the types file"
    );
    let errs = result.unwrap_err();
    assert!(
        errs.iter()
...
fn extends_with_inherited_requires_full_pipeline() {
    ...
    checker
        .check(&ast)
        .expect("checker should accept requires on inherited step");
    let compiler = SkillMdCompiler::new();
...
fn brainstorming_example_imports_resolve() {
    let example_path = concat!(env!("CARGO_MANIFEST_DIR"), "/examples/brainstorming.agent");
...
fn watch_flag_accepted() {
    ...
    assert!(
        help.contains("--watch"),
        "build --help should mention --watch flag"
    );
}
...
fn rebuild_on_change() {
    ...
    assert!(
        rebuild_count >= 1,
        "expected at least 1 rebuild trigger, got output: {:?}",
        output
    );
...
fn budget_flag_trims_output() {
    ...
    let trimmed =
        skillspec_core::budget::trim_to_budget(&mut ast.skills[0].body.contexts, before / 2);
    let after = skillspec_core::budget::estimate_context_tokens(&ast.skills[0].body.contexts);

    assert!(
        !trimmed.is_empty(),
        "should have trimmed at least one context"
    );
    assert!(
        after <= before / 2,
        "after trimming should be within budget: {} <= {}",
        after,
        before / 2
    );
    assert!(
        trimmed[0].priority == Some(skillspec_core::ast::Priority::Optional),
        "lowest priority should be trimmed first"
    );
}
...
fn budget_does_not_trim_step_contexts() {
    ...
    assert!(
        ast.skills[0].body.contexts.is_empty(),
        "body contexts should be trimmed"
    );
    assert!(
        !ast.skills[0].body.steps[0].contexts.is_empty(),
        "step contexts should be preserved"
    );
}
...
fn observe_block_round_trips_through_ir() {
    ...
    let observe = round.skills[0]
        .body
        .observe
        .as_ref()
        .expect("observe should survive serialization");
...
fn lint_clean_skill_no_warnings() {
    ...
    assert!(
        diags.is_empty(),
        "code_review.agent should have zero lint warnings, got: {:?}",
        diags
            .iter()
...
fn migrate_directory_integration() {
    ...
    assert!(
        stdout.contains("1 additional file"),
    ...
    assert!(
        partial_path.exists(),
        "should create .agent.partial at {}",
